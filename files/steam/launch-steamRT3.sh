...
if [ -x "$STEAMROOT/steamrtarm64/pv-runtime/steam-runtime-steamrt-arm64" ]; then
	# we're ready to launch the runtime-service and steam
    log "Starting steam-runtime-launcher-service"
    "$STEAMROOT/steamrtarm64/pv-runtime/steam-runtime-steamrt-arm64/pressure-vessel/bin/steam-runtime-launcher-service" \
		--bus-name com.steampowered.PressureVessel.LaunchAlongsideSwitchdeck \
		--alongside-steam \
		--verbose &
    log "Starting steam inside the steam-runtime"
    "$STEAMROOT/steamrtarm64/pv-runtime/steam-runtime-steamrt-arm64/_v2-entry-point" -- "$STEAMROOT/launch-steam.sh" "$@"
else
